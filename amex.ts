...
const getAccounts = (html: string): Account[] => {
  ...
  const initialStateJSON: any = JSON.parse(
    sandbox.window.__INITIAL_STATE__
  );
...
export async function fetchTransactions(): Promise<Account[]> {
  ...
  if (!username || !password)
    throw new Error(
      'You must provide Amex user and password to fetch data.'
    );
  ...
  for (const [i, button] of otpButtonsDOM.entries()) {
    ...
    if (
      html &&
      html.includes('Einmaliger Verifizierungscode (E-Mail)')
    ) {
      emailAuthBtnIndex = i;
      break;
    }
  ...
  const waitForEmail = mailbox.waitForEmail(
    (email: Email) =>
      email.subject ===
      'Ihr einmaliger American Express Verifizierungscode'
  );
  ...
  const textSplit = text.split('lichen Daten:', 2);
  if (textSplit.length == 2) {
    const codeString = textSplit[1];
    const codeMatches = codeString.match(/[0-9]+/g) || [];
    const potentialCode = codeMatches[0];
    if (potentialCode && potentialCode.length === 6)
      code = potentialCode;
  }

  if (!code)
    throw new Error('OTP code could not be extracted from email');
